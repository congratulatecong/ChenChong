...
package com.cong.chenchong.capture;
...
public final class ViewfinderView extends View {
	...
	private final int flickerPointColor;
	...
	public ViewfinderView(Context context, AttributeSet attrs) {
		...
		// 闪烁小圆点的颜色
		flickerPointColor = resources.getColor(R.color.flicker_points);
	...
	public void onDraw(Canvas canvas) {
		// 中间的扫描框，若要修改扫描框的大小，去CameraManager里面修改
		...
		if (resultBitmap != null) {
			...
			String text = getResources().getString(R.string.scan_tips);
			float textWidth = paint.measureText(text);

			// 画扫描框上面的字
			canvas.drawText(text, (width - textWidth) / 2, (float) (frame.bottom + (float) TEXT_PADDING_TOP * density), paint);
//			canvas.drawText(text, (width - textWidth) / 2, (float) (frame.top - (float) TEXT_PADDING_BOTTOM * density), paint);
			...
			if (currentPossible.isEmpty()) {
				...
				paint.setAlpha(OPAQUE);
				paint.setColor(flickerPointColor);
