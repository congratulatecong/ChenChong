...
public class RollingSquareView extends View {
     ...
     * 方框的圆角半径
     */
    private float mRollRoundCorner;
    private float mFixRoundCorner;
    ...
    public RollingSquareView(Context context) {
        this(context, null);
    }

    public RollingSquareView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public RollingSquareView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.RollingSquareView);
        mLineCount = typedArray.getInteger(R.styleable.RollingSquareView_line_count, 3);
        mRollRoundCorner = typedArray.getFloat(R.styleable.RollingSquareView_roll_round_corner, 10);
        mFixRoundCorner = typedArray.getFloat(R.styleable.RollingSquareView_fix_round_corner, 10);
        ...
        mHalfSquareWidth = typedArray.getDimension(R.styleable.RollingSquareView_half_rect_width, 30);
        mDividerWidth = typedArray.getDimension(R.styleable.RollingSquareView_rect_divider_width, 10);
        mIsClockwise = typedArray.getBoolean(R.styleable.RollingSquareView_is_clockwise, true);
        mStartEmptyPosition = typedArray.getInteger(R.styleable.RollingSquareView_start_empty_position, 0);
    ...
    private void startRoll() {
        ...
        ValueAnimator translateController = createTranslateValueAnimator(currEmptyFixSquare,
                rollSquare);
    ...
    protected void onDraw(Canvas canvas) {
        for (FixSquare mFixSquare : mFixSquares) {
            if (mFixSquare.isShow) {
                canvas.drawRoundRect(mFixSquare.rectF, mFixRoundCorner, mFixRoundCorner, mPaint);
            }
        }
        if (mRollSquare.isShow) {
            canvas.rotate(mIsClockwise ? mRotateDegree : -mRotateDegree, mRollSquare.cx, mRollSquare.cy);
            canvas.drawRoundRect(mRollSquare.rectF, mRollRoundCorner, mRollRoundCorner, mPaint);
        }
    }
