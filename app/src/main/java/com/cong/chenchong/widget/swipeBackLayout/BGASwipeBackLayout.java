...
package com.cong.chenchong.widget.swipeBackLayout;
...
/**
 * BGASwipeBackLayout对于华为的虚拟导航条兼容有问题，
 * 基于BGASwipeBackLayout源码进行了修改
 */
public class BGASwipeBackLayout extends ViewGroup {
     ...
     * 阴影资源 id
     */
    private int mShadowResId = R.drawable.swipe_back_shadow;
    /**
     * 滑动返回时的阴影视图
     */
    ...
    private boolean canScroll(View v, int dx, int x, int y) {
    ...
    private boolean canViewPagerScrollHorizontally(ViewPager p, int dx) {
        return !(dx < 0 && p.getCurrentItem() <= 0 ||
                0 < dx && p.getAdapter() != null && p.getAdapter().getCount() - 1 <= p.getCurrentItem());
    }
    ...
    /**
     * 设置阴影资源 id。默认值为 R.drawable.swipe_back_shadow
     *
     * @param shadowResId
     */
...
//        maxLayoutHeight -= UIUtil.getNavigationBarHeight(getContext());
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (canScroll(this, false, 1, Math.round(ev.getX()), Math.round(ev.getY()))) {
            return false;
        }
