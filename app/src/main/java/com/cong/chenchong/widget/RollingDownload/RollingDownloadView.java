...
public class RollingDownloadView extends View {
    ...
    //定义4个运动的点
    private MovePoint[] movePoints = new MovePoint[4];
    ...
    public RollingDownloadView(Context context) {
        this(context, null);
    }
    ...
    private void initAnimation() {
        ...
        translateAnimation.setDuration(expandSpeed);
    ...
    private class MovePoint {
        ...
        MovePoint(float radius, float moveX, float moveY) {
            this.radius = radius;
            this.moveX = moveX;
            this.moveY = moveY;
            this.startX = moveX;
        ...
        if (status == Status.Normal || status == Status.Start) {
            ...
            if (progress != 100) {
                ...
                for (MovePoint movePoint : movePoints) {
                    if (movePoint.isDraw)
                        canvas.drawCircle(movePoint.moveX, movePoint.moveY, movePoint.radius, textPaint);
                }
            }
    ...
    private float getCircleY(float cx) {
        return (float) (height * 1.0 / 2 - Math.sqrt((height * 1.0 / 2 - dp2px(7)) * (height * 1.0 / 2 - dp2px(7)) - ((width - height * 1.0 / 2) - cx) * ((width - height * 1.0 / 2) - cx)));
    }
    ...
    private float drawMovePoints(float moveX) {
        return (float) (height / 2 + (height / 2 - movePoints[3].radius) * Math.sin(4 * Math.PI * moveX / (width - height) + height / 2));
    }
    ...
    private void initMovePoints() {
        movePoints[0] = new MovePoint(dp2px(4), (float) ((width - height / 2) * 0.88), 0);
        movePoints[1] = new MovePoint(dp2px(3), (float) ((width - height / 2) * 0.83), 0);
        movePoints[2] = new MovePoint(dp2px(2), (float) ((width - height / 2) * 0.78), 0);
        movePoints[3] = new MovePoint(dp2px(5), (float) ((width - height / 2) * 0.70), 0);
        ...
        movePointAnimation.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                for (MovePoint movePoint : movePoints) {
                    movePoint.moveX = movePoint.startX - movePoints[0].startX * value;
                    if (movePoint.moveX <= height / 2) {
                        movePoint.isDraw = false;
                    }
                    movePoint.moveY = drawMovePoints(movePoint.moveX);
                }
                Log.d("TAG", "movePoints[0].moveX:" + movePoints[0].moveX + ",movePoints[0].moveY:" + movePoints[0].moveY);
            }
        });
    ...
    public void setStop(boolean stop) {
        if (status != RollingDownloadView.Status.Load) {
            return;
        }
        if (this.stop == stop) {
            return;
        ...
        if (stop) {
            ...
            if (onProgressStateChangeListener != null) {
                onProgressStateChangeListener.onStopDownload();
            }
        } else {
            loadRotateAnimation.start();
            movePointAnimation.start();
