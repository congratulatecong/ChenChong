package com.cong.chenchong.widget.fish;
...
class FishDrawable extends Drawable {
    static final float HEAD_RADIUS = 30;
    private static final float BODY_LENGTH = HEAD_RADIUS * 3.2f; //第一节身体长度
...
//				mainAngle = currentValue % 360;
     ...
     * 设置身体主轴线方向角度
     */
    void setMainAngle(float mainAngle) {
    ...
    ObjectAnimator getFinsAnimator() {
        return finsAnimator;
    }
    ...
    private void makeBody(Canvas canvas, float headRadius) {
        ...
        //决定胖瘦
        contralLeft = calculatePoint(headPoint, BODY_LENGTH * 0.56f, angle - 130);
        contralRight = calculatePoint(headPoint, BODY_LENGTH * 0.56f, angle + 130);
        mPath.reset();
    ...
    private void makeSegments(Canvas canvas, PointF mainPoint, float segmentRadius, float MP, float fatherAngle) {
        ...
        PointF endPoint = calculatePoint(mainPoint, segementLenght, angle - 180);
